export default function SkeletonCard({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`card ${compact ? 'w-44 sm:w-48' : ''}`}>
      <div className={`relative overflow-hidden ${compact ? 'aspect-[4/5] sm:aspect-square' : 'aspect-square'}`}>
      ...
      <div className={`p-2 ${compact ? 'p-2' : 'p-3'}`}>
        <div className="h-3 bg-cream-100 dark:bg-dark-700 rounded w-2/3 mb-2" />
