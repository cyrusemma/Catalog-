...
export default function StoreNavbar() {
  const { storeSlug, storeName, logoUrl, storeId } = useStoreContext()
  const location = useLocation()
  const allItems = useCartStore(s => s.items)
  const totalItems = useMemo(() => {
    if (storeId) {
      return allItems.filter(i => i.product.store_id === storeId).reduce((sum, i) => sum + i.quantity, 0)
    }
    return allItems.reduce((sum, i) => sum + i.quantity, 0)
  }, [allItems, storeId])
      ...
      <div className="max-w-5xl mx-auto pl-[calc(1rem+env(safe-area-inset-left,0px))] pr-[calc(1rem+env(safe-area-inset-right,0px))] pt-[calc(0.75rem+env(safe-area-inset-top,0px))] pointer-events-auto">
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            <Link to={`/s/${storeSlug}/cart`} className="relative w-8 h-8 rounded-xl flex items-center justify-center hover:bg-brand-400/10 transition-colors pointer-events-auto">
